...
XMatrix mcAnalytical(unsigned long startPath, unsigned long numPaths, unsigned long numSteps, double T, double dt, double sqrt_dt,
                      double mu, double sigma, double S0, XMatrix& S) {
...
int main() {
    unsigned long numPaths = 5e5; // Total number of paths
    unsigned long numSteps = 300; // Number of time steps
    ...
    for (unsigned int i = 0; i < numThreads; ++i) {
        ...
        threads.emplace_back(mcAnalytical, startPath, pathsPerThread, numSteps, T, dt, sqrt_dt, mu, sigma, S0, std::ref(S));
    }
