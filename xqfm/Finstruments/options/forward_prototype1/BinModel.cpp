...
Real BinModel::RiskNeutProb() { return (rate - down) / (up - down); }
...
int BinModel::GetInputData() { // entering data
  ...
  S0 = 1, up = 0.3, down = -0.2, rate = 0.01;
  S0.setGradient(1.0);
  // making sure that 0<S0, -1<D<U, -1<R
  if (S0 <= 0.0 || up <= -1.0 || down <= -1.0 || up <= down || rate <= -1.0) {
...
Real BinModel::GetR() { return rate; }
